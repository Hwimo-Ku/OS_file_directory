...
struct MkdirThreadData {//멀티스레딩을 위해서
...
int ChangeOwnerAll(DirectoryTree* dirTree, char* dirName, char* userName1, char* userName2);
//find
...
//utility
void* GrepThread(void* arg) {
    struct GrepThreadData* thread_data = (struct GrepThreadData*)arg;
    char* file_name = thread_data->file_name;
    char* searching_word = thread_data->searching_word;
...
void* MkdirThread(void* arg) {
    struct MkdirThreadData* data = (struct MkdirThreadData*)arg;
    Makedir(data->dirTree, data->dirName, 'd');
    pthread_exit(NULL);
...
int MovePath(DirectoryTree* dirTree, char* dirPath)
{
    ...
    //dirPath 가 "/" 인 경우 root 로
    if (strcmp(dirPath, "/") == 0) {
        dirTree->current = dirTree->root;
    }
    //dirPath 가 절대경로 혹은 상대경로로 주어질 경우
    else {
        //절대 경로는 시작이 "/"
...
int ChangeOwnerAll(DirectoryTree* dirTree, char* dirName, char* userName1, char* userName2)
{
    TreeNode* tmpNode = NULL;
    TreeNode* tmpNode2 = NULL;
    UserNode* tmpUser1 = NULL;
    UserNode* tmpUser2 = NULL;
    tmpNode = IsExistDir(dirTree, dirName, 'd');
    tmpNode2 = IsExistDir(dirTree, dirName, 'f');
    ...
    if (tmpNode != NULL) {
        ...
        tmpUser2 = IsExistUser(usrList, userName2);
        if (tmpUser1 != NULL && tmpUser2 != NULL) {
            tmpNode->UID = tmpUser1->UID;
            tmpNode->GID = tmpUser2->GID;
        }
        else {
            printf("chown: 잘못된 사용자\n");
            printf("Try 'chown --help' for more information.\n");
            return -1;
        }
    }
...
int Mkdir(DirectoryTree* dirTree, char* cmd)
{
    ...
    str = strtok(NULL, " "); //str 는 옵션 혹은 만들 디렉토리(mkdir 이후 오는 문자열)
    if (strcmp(cmd_copy, "mkdir") == 0) { // mkdir 이후에 아무것도 없으면
    ...
    {
        if (strcmp(str, "-p") == 0) {
            dirName = strtok(NULL, " "); //dirName 은 만들 디렉토리 
            if (dirName == NULL) {
                printf("mkdir: 잘못된 연산자\n");
                printf("Try 'mkdir --help' for more information.\n");
                return -1;
            }
            else //mkdir -p 옵션에서 멀티스레딩으로 처리
            {
            ...
            while (dirName != NULL) {
                val = Movecurrent(dirTree, dirName);
                if (val != 0) {
                    MakeDir(dirTree, dirName, 'd');
                    Movecurrent(dirTree, dirName);
                }
                dirName = strtok(NULL, "/");
            }
            dirTree->current = tmpNode;
            ...
            str = strtok(str, "-"); //str 은 - 다음에 오는 문자
            if (str == NULL) {
                printf("mkdir: 잘못된 연산자\n");
                printf("Try 'mkdir --help' for more information.\n");
                return -1;
    ...
    else { //옵션이 없는 경우 여기서 str 은 만들 디렉토리 이름
        str_multi = strtok(NULL, " "); //여러개 만들면 str_multi != NULL
        if (str_multi == NULL)
        {// 1개만 만들 경우
            strncpy(tmp, str, MAX_DIR);//str 은 만들 디렉토리 이름 혹은 경로
            if (strstr(str, "/") == NULL) { //절대 경로가 아니라면
                MakeDir(dirTree, str, 'd');
                dirTree->current = tmpNode;
                return 0;
            }
            else { //만들 디렉토리가 상대 경로
                strncpy(tmp2, getDir(str), MAX_DIR); //tmp2 는 상대 경로
        ...
        else // 멀티스레딩을 통해서 여러개를 한 번에 만들경우
        {
            str_multi = strtok(cmd_copy, " "); //첫 토큰 : mkdir
            dirName = strtok(NULL, " "); //2번째 토큰 : 만들 첫번째 디렉토리
            ...
            while (dirName != NULL) {
                ...
                threadData->dirTree->root = dirTree->root; // 새로운 값으로 설정
                threadData->dirTree->current = dirTree->current;
                if (num_threads >= MAX_THREAD) { // 수정: num_threads 증가 전에 조건 검사
                    printf("maximum number of threads exceeded\n");
                    free(threadData); // 메모리 해제
                    break;
                }
...
int cd(DirectoryTree* dirTree, char* cmd)
{
    ...
    if (cmd == NULL) {// cd 다음 아무것도안치면 제자리
        strcpy(tmp, usrList->current->dir);
        MovePath(dirTree, tmp);
    }
    else {//cmd 가 상대주소 혹은 절대주소
...
int chown_(DirectoryTree* dirTree, char* cmd)
{
    ...
    if (str2 == NULL) // UID만 변환 cmd=변환자 str1=디렉토리
    {
        ...
        tmpNode = IsExistDir(dirTree, str1, 'd');
        if (tmpNode != NULL) {
    ...
    else if (cmd != NULL && str1 != NULL) //str1,cmd 변환자, str2=디렉토리
    {
        tmpUser = IsExistUser(usrList, cmd);
        if (tmpUser != NULL) {
            strncpy(tmp1, cmd, MAX_NAME);
        ...
        tmpUser = IsExistUser(usrList, str1);
        if (tmpUser != NULL) {
        ...
        tmpNode = IsExistDir(dirTree, str2, 'd');
        if (tmpNode != NULL) {
